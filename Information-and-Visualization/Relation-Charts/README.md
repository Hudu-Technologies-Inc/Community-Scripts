...
- `$MaxRelationMapNodes` — the maximum total node count to allow while adding external relations. Scoped company-owned nodes are always kept; external relations are skipped once adding them would exceed this limit. Set to `0` for unlimited. Defaults to `250`.
